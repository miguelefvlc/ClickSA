...
const val = (state, key) => state.inputs[key] || state.selects[key] || '-';
...
const formatSmartTagsList = (tagsArray, type) => {
    ...
    let ul = tagsArray.map(str => {
        ...
        if (m) { 
            if (type === 'OD') { return { text: [ { text: m[1], bold: true }, ` - ${m[2]}` ] }; }
...
const buildTemporalitzacio = (state) => {
...
const buildPerfilEixidaSingleTable = (activeCCs) => {
...
const buildMesuresEspecifiques = (state) => {
    let content = [];
...
const buildSequenceTableBody = (state) => {
...
export function generatePDF() {
    ...
    const docContent = [
        { text: `SITUACIÓ D'APRENENTATGE Nº ${val(state, 'input-sa-num')}: ${val(state, 'input-sa-title').toUpperCase()}`, style: 'header' }
    ];
    ...
    if (!isBlockDisabled(1)) {
        ...
        docContent.push({ table: { widths: ['10%', '25%', '10%', '15%', '10%', '10%', '10%', '10%'], body: [[{text:'Àrea', style:'th'}, val(state, 'input-area'), {text:'Cicle', style:'th'}, val(state, 'input-cicle'), {text:'Nivell', style:'th'}, val(state, 'input-curs'), {text:'Trim.', style:'th'}, val(state, 'input-trimestre')]] }, layout: standardTableLayout, margin: [0, 0, 0, 8] });
        docContent.push({ table: { widths: ['100%'], body: [[{text: 'TEMPORALITZACIÓ', style: 'thCenter'}], [buildTemporalitzacio(state)]] }, layout: standardTableLayout, margin: [0, 0, 0, 8] });
        docContent.push({ table: { widths: ['75%', '25%'], body: [ [{text: 'Repte, Problema, Necessitat', style: 'th'}, {text: 'ODS', style: 'thCenter'}], [{text: val(state, 'input-repte'), margin: [4,4,4,4]}, Object.assign({}, formatSmartTagsList(state.tags['tags-ods'], 'ODS'), { rowSpan: 3 })], [{text: 'Producte Final', style: 'th'}, ''], [{text: val(state, 'input-tasca'), margin: [4,4,4,4]}, ''] ] }, layout: standardTableLayout });
        sectionNum++;
    }
    ...
    if (!isBlockDisabled(2)) {
        ...
        docContent.push({ table: { widths: ['100%'], body: [[{text:'Perfil d\'Eixida (Descriptors Operatius)', style:'thCenter'}], [buildPerfilEixidaSingleTable(activeCCs)]] }, layout: standardTableLayout, margin: [0, 0, 0, 8] });
    ...
    if (!isBlockDisabled(5)) {
        ...
        docContent.push(...buildMesuresEspecifiques(state));
        sectionNum++;
    }

    if (!isBlockDisabled(6)) {
        docContent.push(buildSectionTitle(sectionNum.toString(), 'Seqüència Didàctica'));
        docContent.push({ table: { widths: ['*', 40], body: buildSequenceTableBody(state) }, layout: standardTableLayout });
        sectionNum++;
    }
    ...
    const saNum = val(state, 'input-sa-num') === "-" ? "___" : val(state, 'input-sa-num');
    const saTitle = val(state, 'input-sa-title') === "-" ? "Sense_titol" : val(state, 'input-sa-title');
...
export function importJSON(e) {
    ...
    reader.onload = evt => {
        try {
            ...
            if (!content || typeof content !== 'object') throw new Error();
            
            // Retrocompatibilidad: Asegurar que existan las claves mínimas esperadas
            content.inputs = content.inputs || {};
            content.selects = content.selects || {};
            content.tags = content.tags || {};
            content.destinataris = content.destinataris || [];
            content.sequence = content.sequence || [];
            content.temporalitzacio = content.temporalitzacio || [];
            
            localStorage.setItem('clickSA_state', JSON.stringify(content));
            location.reload();
        } catch (err) { 
            alert("❌ El fitxer seleccionat no és un document vàlid de ClickSA."); 
        }
    };
